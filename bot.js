...
client.on('ready', () => {
  ...
  console.log('')
  console.log('By [HC] ShadowHacker')
  console.log('')  
...
client.on('message', msg => {
  ...
  let args = msg.content.split(" ").slice(1);
    ...
    if (textxt == "") {
        msg.delete().then
    msg.channel.send("```اكتب عدد الرسائل التي تريد حدفها ```").then(m => m.delete(3000));
} else {
    msg.delete().then
    msg.delete().then
    msg.channel.bulkDelete(textxt);
        msg.channel.send("```عدد الرسائل التي مسحتها هي: " + textxt + "```").then(m => m.delete(3000));
        }    
    }
}
});
...
client.on('message', message => {
    if(message.content.includes('discord.gg')){
        ...
        message.delete()
    return message.reply(`** ممنوع نشر الروابط ادا تنشر مره تانيه حتاد ميوت :angry: ! **`)
    }
}
});
...
امر خاص لصاحب البوت لوحده  :lock:
-ownerbot
...
if (message.content.startsWith('---superchat')){
